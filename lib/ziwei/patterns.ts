 ...
 *  - 倪海厦《天纪》紫微斗数讲义
 *  - 王亭之《安星法及推断实例》（中州派独特格局+暗合宫+煞星量化）
 */

import type { ZiweiChart, Palace, Star } from './types';
import type { WangPattern } from './types';
import { detectWangPatterns } from './wang-analysis';
...
export function detectPatterns(chart: ZiweiChart): Pattern[] {
  ...
  // ── 王亭之《安星法及推断实例》中州派格 ──
  // 包含：科权禄拱命、禄马交驰、辅弼拱主、魁钺夹贵、
  //       火铃夹命、羊陀夹命、日月反背、杀破狼、巨火擎羊、泛水桃花
  const wangPats = detectWangPatterns(chart);
  for (const wp of wangPats) {
    patterns.push({
      name: wp.name,
      level: wp.type === 'excellent' ? 'excellent' as const
            : wp.type === 'good' ? 'good' as const
            : wp.type === 'caution' ? 'caution' as const
            : 'caution' as const,
      description: wp.description,
      palaces: wp.relatedPalaces,
      conditions: { required: [wp.name], bonus: [], breaking: [] },
      source: wp.source,
    });
  }
